...
import {
  deliveryOptions,
  getDateStringFormattedForDeliveryOption,
...
function renderCartSummaryHTML() {
  ...
  cart.cartItems.forEach((cartItem) => {
    ...
    const dateString = getDateStringFormattedForDeliveryOption(
...
function renderDeliveryOptionsHTML(product, cartItem) {
  ...
  deliveryOptions.forEach((deliveryOption) => {
    ...
    deliveryOptionsHTML += `
     <div class="delivery-option js-delivery-option-${product.id}-${
      deliveryOption.id
    }">
        <input type="radio" ${
          isChecked ? "checked" : ""
      ...
      product.id
    }-${deliveryOption.id}" data-delivery-option-id="${
      deliveryOption.id
    }" data-product-id="${product.id}" name="delivery-option-${product.id}">
        <div>
...
export function displayOrderSummary() {
  ...
  orderSummaryElement.innerHTML = renderCartSummaryHTML(cart);
}
...
function getNewQuantityFromInput(productId) {
  const newQuantityElement = document.querySelector(
...
function addClickEventForDeliveryOptions() {
  document.querySelectorAll(".js-delivery-option-input")?.forEach((option) => {
    addEventToElement(option, "click", () => {
      ...
      cart.updateDeliveryOptionIdInCart(deliveryOptionId, productId);
      renderCheckout();
    });
  });
}
