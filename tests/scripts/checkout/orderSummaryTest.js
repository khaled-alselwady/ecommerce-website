...
describe('test suite: renderOrderSummary', () => {
  ...
  it('updates the delivery option', () => {
    ...
    expect(document.querySelector(`.js-delivery-option-input-${productId1}-${deliveryOptionId}`).checked).toEqual(true);
    expect(cart.length).toEqual(2);
    expect(cart[0].productId).toEqual(productId1);
    expect(cart[0].deliveryOptionId).toEqual(deliveryOptionId);

    expect(document.querySelector('.js-total-shipping-price-cents').innerText).toEqual('$14.98');
    expect(document.querySelector('.js-order-total-price-cent').innerText).toEqual('$63.50');
  });
